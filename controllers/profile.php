...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($password)) { // поля, заполняемые пользователем не пустые
        ...
        $validation = validation ($id, $password, $hashedPassword, $name, $bd_name, $tel, $bd_tel, $email, $bd_email, $new_password, $is_update, $connection);
        if(!$validation) {
            return ;
        }        
...
function validation($id, $password, $hashedPassword, $name, $bd_name, $tel, $bd_tel, $email, $bd_email, $new_password, $is_update, $connection)
{
